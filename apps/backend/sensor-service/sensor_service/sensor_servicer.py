...
class SensorServicer(sensor_pb2_grpc.SensorServiceServicer):
    def __init__(self):
        self._sensor_reading_mgr = SensorReadingMgr(db_mgr)

    async def SubmitReading(self, request, context):
        reading = {
        ...
        try:
            logger.info(
                f"Creating reading and outbox event: event_id={request.event_id}, temp={request.temperature}C"
            )
            self._sensor_reading_mgr.create_reading_and_outbox_event(
                event_id=request.event_id,
            ...
            logger.info(
                f"Sensor reading accepted for processing: event_id={request.event_id}"
            )
        except DuplicateEventError:
            logger.warning(
                f"Duplicate sensor reading received: event_id={request.event_id}"
            )
        except Exception as e:
            logger.error(
                f"Failed to create sensor reading event: event_id={request.event_id}, error={e!s}"
            )
    ...
    def ListReadings(self, request, context):
        try:
            logger.info(
                f"Retrieving sensor readings: limit={request.limit}"
            )
            readings = self._sensor_reading_mgr.get_readings(request.limit)
            logger.info(f"Retrieved {len(readings)} sensor readings")

            for reading in readings:
                yield _to_proto_reading(reading)
        except Exception as e:
            logger.error(f"Failed to retrieve sensor readings: {e!s}")
            raise HTTPException(
                status_code=503, detail=f"Database unavailable: {e!s}"
            )
