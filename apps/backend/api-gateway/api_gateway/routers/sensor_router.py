...
sensorapirouter = APIRouter(prefix="/api/v1/sensor", tags=["sensor-data"])
...
async def create_sensor_reading(
    ...
    try:
        ...
        sensor_data_publisher.publish_readings(reading)
        ...
        logger.error(f"Failed to create sensor reading event: event_id={event_id}, error={e!s}")
...
async def get_sensor_data(
    limit: int = Query(
        200, ge=1, le=1000, description="Maximum number of sensor readings to return"
    ),
):
    try:
        logger.info(f"Retrieving sensor readings: limit={limit}")
        sensor_client = SensorClient()
        readings = sensor_client.list_readings(limit)
