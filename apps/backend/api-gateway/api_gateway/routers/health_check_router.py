...
async def api_health():
    ...
    return HealthCheckResponse(status="healthy", service="backend", version="1.0.0")
...
async def api_liveness():
    ...
    return HealthCheckResponse(status="alive", service="backend", version="1.0.0")
...
async def api_readiness():
    ...
    try:
        ...
        return HealthCheckResponse(status="ready", service="backend", version="1.0.0")

    except Exception as e:
        logger.error(f"Readiness check failed: {e!s}")
        raise HTTPException(status_code=503, detail=f"Service not ready: {e!s}")
