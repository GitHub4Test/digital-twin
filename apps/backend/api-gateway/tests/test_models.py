...
class TestSensorReading(unittest.TestCase):
    """Test suite for SensorReading model"""
    ...
    def test_extreme_valid_temperatures(self):
        ...
        now = datetime.now().isoformat()
        cold = SensorReading(timestamp=now, temperature=-40.0, humidity=50.0)
        hot = SensorReading(timestamp=now, temperature=140.0, humidity=50.0)
    ...
    def test_humidity_boundary_conditions(self):
        ...
        now = datetime.now().isoformat()
        dry = SensorReading(timestamp=now, temperature=25.0, humidity=5.0)
        humid = SensorReading(timestamp=now, temperature=25.0, humidity=95.0)
