...
from api_service.routes import common
...
class TestSensorReadingsValidation(unittest.TestCase):
    ...
    def test_create_reading_missing_timestamp(self):
        """POST /api/v1/readings without timestamp should fail"""
        payload = {
            "temperature": 24.5,
            "humidity": 61.2
        }
    ...
    def test_create_reading_missing_temperature(self):
        """POST /api/v1/readings without temperature should fail"""
    ...
    def test_create_reading_missing_humidity(self):
        """POST /api/v1/readings without humidity should fail"""
        now = datetime.now().isoformat()
        payload = {
            "timestamp": now,
            "temperature": 24.5
    ...
    def test_create_reading_temperature_at_min_boundary(self):
        ...
        response = client.post("/api/v1/readings", json=payload)
        # Will be 201 if DB works, 500 if DB not initialized (acceptable for validation test)
    ...
    def test_event_endpoint_missing_timestamp(self):
        ...
        response = client.post("/api/v1/readings/event", json=payload)
        self.assertEqual(response.status_code, 422)
...
class TestGetSensorDataEndpoint(unittest.TestCase):
    ...
    def test_get_sensor_data_accepts_no_params(self):
        """GET /api/v1/readings should work without parameters"""
    ...
    def test_get_sensor_data_with_limit_1(self):
        """GET /api/v1/readings?limit=1 should be valid"""
        response = client.get("/api/v1/readings?limit=1")
        self.assertIn(response.status_code, [200, 500, 503])

    def test_get_sensor_data_with_limit_1000(self):
        """GET /api/v1/readings?limit=1000 should be valid"""
        response = client.get("/api/v1/readings?limit=1000")
        self.assertIn(response.status_code, [200, 500, 503])

    def test_get_sensor_data_limit_0_invalid(self):
        """GET /api/v1/readings?limit=0 should fail"""
        response = client.get("/api/v1/readings?limit=0")
        self.assertEqual(response.status_code, 422)

    def test_get_sensor_data_limit_1001_invalid(self):
        """GET /api/v1/readings?limit=1001 should fail"""
        response = client.get("/api/v1/readings?limit=1001")
        self.assertEqual(response.status_code, 422)

    def test_get_sensor_data_limit_string_invalid(self):
        """GET /api/v1/readings?limit=abc should fail"""
        response = client.get("/api/v1/readings?limit=abc")
        self.assertEqual(response.status_code, 422)

    def test_get_sensor_data_limit_float(self):
        """GET /api/v1/readings?limit=50.5 might be coerced to int"""
        response = client.get("/api/v1/readings?limit=50.5")
...
class TestDeleteSensorDataEndpoint(unittest.TestCase):
    ...
    def test_delete_sensor_data_returns_message_response(self):
        """DELETE /api/v1/readings should return message"""
        response = client.delete("/api/v1/readings")
        if response.status_code == 200:
            data = response.json()
    ...
    def test_delete_sensor_data_accept_no_body(self):
        """DELETE /api/v1/readings should not require body"""
        response = client.delete("/api/v1/readings")
...
class TestAPIErrorHandling(unittest.TestCase):
    ...
    def test_put_method_not_allowed(self):
        ...
        response = client.put("/api/v1/readings")
        self.assertEqual(response.status_code, 405)

    def test_patch_method_not_allowed(self):
        """PATCH method should not be allowed"""
        response = client.patch("/api/v1/readings")
        self.assertEqual(response.status_code, 405)
